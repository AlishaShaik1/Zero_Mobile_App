...
CONTENT = r"""// ring_audio_pipeline.dart
//
...
//      and forwarded to AI — reply sent back to ring OLED
//
// ONLY the voice-transfer part has been rebuilt.
// Everything else (BLE connection, agent, tools, chat UI) is unchanged.
...
class RingAudioPipeline {
  ...
  // ── PCM accumulator ───────────────────────────────────────────────────────
  // All PCM chunks from the ring are collected here.
  // When silence is detected the full buffer is POSTed to Deepgram REST.
  ...
  void _handleAudioChunk(Uint8List pcm) {
    ...
    // First real audio without a start marker → auto-start
    if (!_isStreamingFromRing && !_processingUtterance) {
      _startRingListening();
    }
  ...
  void _finalizeSpeech() {
    ...
    // Require at least 200 ms of audio (6400 bytes at 16 kHz 16-bit mono)
    if (pcmBytes < 6400) {
  ...
  Future<void> _transcribeAndProcess(Uint8List pcm) async {
    ...
    try {
      // Wrap raw PCM in a 44-byte WAV header — Deepgram handles it reliably
    ...
    // Show transcript on phone immediately ← THIS IS WHAT THE USER SEES
  ...
  // ==========================================================================
  // AI Processing (unchanged — agent → GLM → reply)
  // ==========================================================================
  ...
  /// Build a standard 44-byte WAV header for 16 kHz 16-bit mono PCM.
