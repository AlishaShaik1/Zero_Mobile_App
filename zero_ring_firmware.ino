...
// Tracks the negotiated ATT MTU of the phone connection.
// Default 23 means a 240-byte audio notification would be SILENTLY DROPPED
// by the NimBLE stack — the classic "ring says Listening, phone shows
// nothing" failure. streamAudioChunk() sizes chunks to fit the MTU.
static int lastAttMtu = 23;

class ZeroServerCallbacks : public NimBLEServerCallbacks {
  void onConnect(NimBLEServer* pServer, NimBLEConnInfo& connInfo) override {
    lastAttMtu = pServer->getAttMtu();
    if (lastAttMtu < 23) lastAttMtu = 23;
    Serial.printf("BLE connected (ATT MTU=%d)\n", lastAttMtu);
  }
...
void setupBLE() {
  ...
  bleServer->setCallbacks(new ZeroServerCallbacks(), true);
...
void streamAudioChunk() {
  ...
  // Size the notification to the negotiated MTU (max value = MTU - 3).
  // With MTU 517 this is the full 240-byte fast path; with a small MTU the
  // chunks shrink so they are never dropped (audio just streams slower).
  int maxBytes = lastAttMtu - 3;
  if (maxBytes < 2) maxBytes = 2;
  int maxSamples = maxBytes / 2;
  int capSamples = (int)(sizeof(chunk) / sizeof(chunk[0]));
  if (maxSamples > capSamples) maxSamples = capSamples;
  if (maxSamples < 1) maxSamples = 1;
  size_t wantBytes = (size_t)maxSamples * sizeof(int16_t);
  size_t bytesRead = 0;
  i2s_read(I2S_PORT_MIC, chunk, wantBytes, &bytesRead, portMAX_DELAY);
